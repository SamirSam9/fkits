...
async def show_cart(user_id, language, message):
    cart = user_carts.get(user_id, [])# ================== ХРАНЕНИЕ ДАННЫХ В ПАМЯТИ ==================
user_sessions = {}
user_selections = {}
user_carts = {}
support_requests = {}
...
async def handle_location(message: types.Message):
    ...
    save_user(
        ...
        language=language,
        region='tashkent',
        post_office=f"Геолокация: {lat}, {lon}"
    )

    # Обновляем сессию
    user_sessions[user_id]['step'] = 'main_menu'
    user_sessions[user_id]['post_office'] = f"Геолокация: {lat}, {lon}"

    if language == 'ru':
        await message.answer(
            "📍 Геолокация получена! Курьер свяжется с вами для уточнения адреса.",
            reply_markup=get_main_menu(language)
        )
    else:
        await message.answer(
            "📍 Geolokatsiya qabul qilindi! Kuryer manzilni aniqlash uchun siz bilan bog'lanadi.",
            reply_markup=get_main_menu(language)
        )
...
async def show_help(message: types.Message):
    ...
    # Если пользователь хочет задать вопрос
    support_requests[message.from_user.id] = {'waiting_question': True}
...
async def handle_support_question(message: types.Message):
    ...
    if user_id in support_requests and support_requests[user_id].get('waiting_question'):
        ...
        if user:
            ...
            if language == 'ru':
                await message.answer("✅ Ваш вопрос отправлен! Мы ответим в ближайшее время.")
            else:
                await message.answer("✅ Savolingiz yuborildi! Tez orada javob beramiz.")
